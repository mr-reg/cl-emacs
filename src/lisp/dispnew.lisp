...
(defun* ding () "Beep, or flash the screen.
Also, unless an argument is given,
terminate any keyboard macro currently executing.

(fn &optional ARG)"
  (error ’unimplemented-error))
(defun* display--update-for-mouse-movement () "Handle mouse movement detected by Lisp code.
...
(fn MOUSE-X MOUSE-Y)"
  (error ’unimplemented-error))
(defun* frame-or-buffer-changed-p () "Return non-nil if the frame and buffer state appears to have changed.
...
(fn &optional VARIABLE)"
  (error ’unimplemented-error))
(defun* internal-show-cursor () "Set the cursor-visibility flag of WINDOW to SHOW.
...
(fn WINDOW SHOW)"
  (error ’unimplemented-error))
(defun* internal-show-cursor-p () "Value is non-nil if next redisplay will display a cursor in WINDOW.
WINDOW nil or omitted means report on the selected window.

(fn &optional WINDOW)"
  (error ’unimplemented-error))
(defun* open-termscript () "Start writing all terminal output to FILE as well as the terminal.
FILE = nil means just close any termscript file currently open.

(fn FILE)"
  (error ’unimplemented-error))
(defun* redisplay () "Perform redisplay.
...
(fn &optional FORCE)"
  (error ’unimplemented-error))
(defun* redraw-display () "Clear and redisplay all visible frames.

(fn)"
  (error ’unimplemented-error))
(defun* redraw-frame () "Clear frame FRAME and output again what is supposed to appear on it.
If FRAME is omitted or nil, the selected frame is used.

(fn &optional FRAME)"
  (error ’unimplemented-error))
(defun* send-string-to-terminal () "Send STRING to the terminal without alteration.
...
(fn STRING &optional TERMINAL)"
  (error ’unimplemented-error))
(defun* sleep-for () "Pause, without updating display, for SECONDS seconds.
