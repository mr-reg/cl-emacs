...
(defun* ccl-execute () "Execute CCL-PROGRAM with registers initialized by REGISTERS.
...
(defun* ccl-execute-on-string () "Execute CCL-PROGRAM with initial STATUS on STRING.
...
(fn CCL-PROGRAM STATUS STRING &optional CONTINUE UNIBYTE-P)"
  (error ’unimplemented-error))
(defun* ccl-program-p () "Return t if OBJECT is a CCL program name or a compiled CCL program code.
See the documentation of ‘define-ccl-program’ for the detail of CCL program.

(fn OBJECT)"
  (error ’unimplemented-error))
(defun* register-ccl-program () "Register CCL program CCL-PROG as NAME in ‘ccl-program-table’.
...
(fn NAME CCL-PROG)"
  (error ’unimplemented-error))
(defun* register-code-conversion-map () "Register SYMBOL as code conversion map MAP.
