...
(defun* eval-buffer () "Execute the accessible portion of current buffer as Lisp code.
...
(defun* eval-region () "Execute the region as Lisp code.
...
(fn START END &optional PRINTFLAG READ-FUNCTION)"
  (error ’unimplemented-error))
(defun* get-file-char () "Don’t use this yourself.

(fn)"
  (error ’unimplemented-error))
(defun* get-load-suffixes () "Return the suffixes that ‘load’ should try if a suffix is required.
This uses the variables ‘load-suffixes’ and ‘load-file-rep-suffixes’.

(fn)"
  (error ’unimplemented-error))
(defun* intern () "Return the canonical symbol whose name is STRING.
...
(fn STRING &optional OBARRAY)"
  (error ’unimplemented-error))
(defun* intern-soft () "Return the canonical symbol named NAME, or nil if none exists.
...
(fn NAME &optional OBARRAY)"
  (error ’unimplemented-error))
(defun* load () "Execute a file of Lisp code named FILE.
...
(fn FILE &optional NOERROR NOMESSAGE NOSUFFIX MUST-SUFFIX)"
  (error ’unimplemented-error))
(defun* locate-file-internal () "Search for FILENAME through PATH.
...
(fn FILENAME PATH &optional SUFFIXES PREDICATE)"
  (error ’unimplemented-error))
(defun* lread--substitute-object-in-subtree () "In OBJECT, replace every occurrence of PLACEHOLDER with OBJECT.
...
(fn OBJECT PLACEHOLDER COMPLETED)"
  (error ’unimplemented-error))
(defun* mapatoms () "Call FUNCTION on every symbol in OBARRAY.
OBARRAY defaults to the value of ‘obarray’.

(fn FUNCTION &optional OBARRAY)"
  (error ’unimplemented-error))
(defun* read () "Read one Lisp expression as text from STREAM, return as Lisp object.
...
(fn &optional STREAM)"
  (error ’unimplemented-error))
(defun* read-char () "Read a character event from the command input (keyboard or macro).
...
(fn &optional PROMPT INHERIT-INPUT-METHOD SECONDS)"
  (error ’unimplemented-error))
(defun* read-char-exclusive () "Read a character event from the command input (keyboard or macro).
...
(fn &optional PROMPT INHERIT-INPUT-METHOD SECONDS)"
  (error ’unimplemented-error))
(defun* read-event () "Read an event object from the input stream.
...
(fn &optional PROMPT INHERIT-INPUT-METHOD SECONDS)"
  (error ’unimplemented-error))
(defun* read-from-string () "Read one Lisp expression which is represented as text by STRING.
...
(fn STRING &optional START END)"
  (error ’unimplemented-error))
(defun* read-positioning-symbols () "Read one Lisp expression as text from STREAM, return as Lisp object.
...
(fn &optional STREAM)"
  (error ’unimplemented-error))
(defun* unintern () "Delete the symbol named NAME, if any, from OBARRAY.
