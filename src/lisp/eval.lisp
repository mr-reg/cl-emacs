...
(defun* and () "Eval args until one of them yields nil, then return nil.
The remaining args are not evalled at all.
If no arg yields nil, return the last arg’s value.

(fn CONDITIONS...)"
  (error ’unimplemented-error))
(defun* apply () "Call FUNCTION with our remaining args, using our last arg as list of args.
...
(fn FUNCTION &rest ARGUMENTS)"
  (error ’unimplemented-error))
(defun* autoload () "Define FUNCTION to autoload from FILE.
...
(fn FUNCTION FILE &optional DOCSTRING INTERACTIVE TYPE)"
  (error ’unimplemented-error))
(defun* autoload-do-load () "Load FUNDEF which should be an autoload.
...
(fn FUNDEF &optional FUNNAME MACRO-ONLY)"
  (error ’unimplemented-error))
(defun* backtrace--frames-from-thread () "Return the list of backtrace frames from current execution point in THREAD.
...
(fn THREAD)"
  (error ’unimplemented-error))
(defun* backtrace--locals () "Return names and values of local variables of a stack frame.
NFRAMES and BASE specify the activation frame to use, as in ‘backtrace-frame’.

(fn NFRAMES &optional BASE)"
  (error ’unimplemented-error))
(defun* backtrace-debug () "Set the debug-on-exit flag of eval frame LEVEL levels down to FLAG.
The debugger is entered when that frame exits, if the flag is non-nil.

(fn LEVEL FLAG)"
  (error ’unimplemented-error))
(defun* backtrace-eval () "Evaluate EXP in the context of some activation frame.
NFRAMES and BASE specify the activation frame to use, as in ‘backtrace-frame’.

(fn EXP NFRAMES &optional BASE)"
  (error ’unimplemented-error))
(defun* backtrace-frame--internal () "Call FUNCTION on stack frame NFRAMES away from BASE.
Return the result of FUNCTION, or nil if no matching frame could be found.

(fn FUNCTION NFRAMES BASE)"
  (error ’unimplemented-error))
(defun* catch () "Eval BODY allowing nonlocal exits using ‘throw’.
...
(fn TAG BODY...)"
  (error ’unimplemented-error))
(defun* commandp () "Non-nil if FUNCTION makes provisions for interactive calling.
...
(fn FUNCTION &optional FOR-CALL-INTERACTIVELY)"
  (error ’unimplemented-error))
(defun* cond () "Try each clause until one succeeds.
...
(fn CLAUSES...)"
  (error ’unimplemented-error))
(defun* condition-case () "Regain control when an error is signaled.
...
(fn VAR BODYFORM &rest HANDLERS)"
  (error ’unimplemented-error))
(defun* default-toplevel-value () "Return SYMBOL’s toplevel default value.
\"Toplevel\" means outside of any let binding.

(fn SYMBOL)"
  (error ’unimplemented-error))
(defun* defconst () "Define SYMBOL as a constant variable.
...
(fn SYMBOL INITVALUE [DOCSTRING])"
  (error ’unimplemented-error))
(defun* defconst-1 () "Like ‘defconst’ but as a function.
More specifically, behaves like (defconst SYM ’INITVALUE DOCSTRING).

(fn SYM INITVALUE &optional DOCSTRING)"
  (error ’unimplemented-error))
(defun* defvar () "Define SYMBOL as a variable, and return SYMBOL.
...
(fn SYMBOL &optional INITVALUE DOCSTRING)"
  (error ’unimplemented-error))
(defun* defvar-1 () "Like ‘defvar’ but as a function.
More specifically behaves like (defvar SYM ’INITVALUE DOCSTRING).

(fn SYM INITVALUE &optional DOCSTRING)"
  (error ’unimplemented-error))
(defun* defvaralias () "Make NEW-ALIAS a variable alias for symbol BASE-VARIABLE.
...
(fn NEW-ALIAS BASE-VARIABLE &optional DOCSTRING)"
  (error ’unimplemented-error))
(defun* eval () "Evaluate FORM and return its value.
...
(fn FORM &optional LEXICAL)"
  (error ’unimplemented-error))
(defun* fetch-bytecode () "If byte-compiled OBJECT is lazy-loaded, fetch it now.

(fn OBJECT)"
  (error ’unimplemented-error))
(defun* func-arity () "Return minimum and maximum number of args allowed for FUNCTION.
...
(fn FUNCTION)"
  (error ’unimplemented-error))
(defun* funcall () "Call first argument as a function, passing remaining arguments to it.
Return the value that function returns.
Thus, (funcall \\=’cons \\=’x \\=’y) returns (x . y).

(fn FUNCTION &rest ARGUMENTS)"
  (error ’unimplemented-error))
(defun* funcall-with-delayed-message () "Like ‘funcall’, but display MESSAGE if FUNCTION takes longer than TIMEOUT.
...
(fn TIMEOUT MESSAGE FUNCTION)"
  (error ’unimplemented-error))
(defun* function () "Like ‘quote’, but preferred for objects which are functions.
...
(fn ARG)"
  (error ’unimplemented-error))
(defun* functionp () "Return t if OBJECT is a function.
...
(fn OBJECT)"
  (error ’unimplemented-error))
(defun* if () "If COND yields non-nil, do THEN, else do ELSE...
...
(fn COND THEN ELSE...)"
  (error ’unimplemented-error))
(defun* internal--define-uninitialized-variable () "Define SYMBOL as a variable, with DOC as its docstring.
This is like ‘defvar’ and ‘defconst’ but without affecting the variable’s
value.

(fn SYMBOL &optional DOC)"
  (error ’unimplemented-error))
(defun* internal-make-var-non-special () "Internal function.

(fn SYMBOL)"
  (error ’unimplemented-error))
(defun* let () "Bind variables according to VARLIST then eval BODY.
...
(fn VARLIST BODY...)"
  (error ’unimplemented-error))
(defun* macroexpand () "Return result of expanding macros at top level of FORM.
...
(fn FORM &optional ENVIRONMENT)"
  (error ’unimplemented-error))
(defun* mapbacktrace () "Call FUNCTION for each frame in backtrace.
...
(fn FUNCTION &optional BASE)"
  (error ’unimplemented-error))
(defun* or () "Eval args until one of them yields non-nil, then return that value.
The remaining args are not evalled at all.
If all args return nil, return nil.

(fn CONDITIONS...)"
  (error ’unimplemented-error))
(defun* prog1 () "Eval FIRST and BODY sequentially; return value from FIRST.
The value of FIRST is saved during the evaluation of the remaining args,
whose values are discarded.

(fn FIRST BODY...)"
  (error ’unimplemented-error))
(defun* progn () "Eval BODY forms sequentially and return value of last one.

(fn BODY...)"
  (error ’unimplemented-error))
(defun* quote () "Return the argument, without evaluating it.  ‘(quote x)’ yields ‘x’.
...
(fn ARG)"
  (error ’unimplemented-error))
(defun* run-hook-with-args () "Run HOOK with the specified arguments ARGS.
...
(fn HOOK &rest ARGS)"
  (error ’unimplemented-error))
(defun* run-hook-with-args-until-failure () "Run HOOK with the specified arguments ARGS.
HOOK should be a symbol, a hook variable.  The value of HOOK
may be nil, a function, or a list of functions.  Call each
...
(fn HOOK &rest ARGS)"
  (error ’unimplemented-error))
(defun* run-hook-with-args-until-success () "Run HOOK with the specified arguments ARGS.
HOOK should be a symbol, a hook variable.  The value of HOOK
may be nil, a function, or a list of functions.  Call each
function in order with arguments ARGS, stopping at the first
...
(fn HOOK &rest ARGS)"
  (error ’unimplemented-error))
(defun* run-hook-wrapped () "Run HOOK, passing each function through WRAP-FUNCTION.
...
(fn HOOK WRAP-FUNCTION &rest ARGS)"
  (error ’unimplemented-error))
(defun* run-hooks () "Run each hook in HOOKS.
...
(fn &rest HOOKS)"
  (error ’unimplemented-error))
(defun* set-default-toplevel-value () "Set SYMBOL’s toplevel default value to VALUE.
\"Toplevel\" means outside of any let binding.

(fn SYMBOL VALUE)"
  (error ’unimplemented-error))
(defun* setq () "Set each SYM to the value of its VAL.
...
(fn [SYM VAL]...)"
  (error ’unimplemented-error))
(defun* signal () "Signal an error.  Args are ERROR-SYMBOL and associated DATA.
...
(fn ERROR-SYMBOL DATA)"
  (error ’unimplemented-error))
(defun* special-variable-p () "Return non-nil if SYMBOL’s global binding has been declared special.
A special variable is one that will be bound dynamically, even in a
context where binding is lexical by default.

(fn SYMBOL)"
  (error ’unimplemented-error))
(defun* throw () "Throw to the catch for TAG and return VALUE from it.
Both TAG and VALUE are evalled.

(fn TAG VALUE)"
  (error ’unimplemented-error))
(defun* unwind-protect () "Do BODYFORM, protecting with UNWINDFORMS.
...
(fn BODYFORM UNWINDFORMS...)"
  (error ’unimplemented-error))
(defun* while () "If TEST yields non-nil, eval BODY... and repeat.
