...
(defun* backward-prefix-chars () "Move point backward over any number of chars with prefix syntax.
This includes chars with expression prefix syntax class (\\=’) and those with
the prefix syntax flag (p).

(fn)"
  (error ’unimplemented-error))
(defun* char-syntax () "Return the syntax code of CHARACTER, described by a character.
...
(fn CHARACTER)"
  (error ’unimplemented-error))
(defun* copy-syntax-table () "Construct a new syntax table and return it.
It is a copy of the TABLE, which defaults to the standard syntax table.

(fn &optional TABLE)"
  (error ’unimplemented-error))
(defun* forward-comment () "Move forward across up to COUNT comments.  If COUNT is negative, move backward.
...
(fn COUNT)"
  (error ’unimplemented-error))
(defun* forward-word () "Move point forward ARG words (backward if ARG is negative).
...
(fn &optional ARG)"
  (error ’unimplemented-error))
(defun* internal-describe-syntax-value () "Insert a description of the internal syntax description SYNTAX at point.

(fn SYNTAX)"
  (error ’unimplemented-error))
(defun* matching-paren () "Return the matching parenthesis of CHARACTER, or nil if none.

(fn CHARACTER)"
  (error ’unimplemented-error))
(defun* modify-syntax-entry () "Set syntax for character CHAR according to string NEWENTRY.
...
(fn CHAR NEWENTRY &optional SYNTAX-TABLE)"
  (error ’unimplemented-error))
(defun* parse-partial-sexp () "Parse Lisp syntax starting at FROM until TO; return status of parse at TO.
...
(fn FROM TO &optional TARGETDEPTH STOPBEFORE OLDSTATE COMMENTSTOP)"
  (error ’unimplemented-error))
(defun* scan-lists () "Scan from character number FROM by COUNT lists.
...
(fn FROM COUNT DEPTH)"
  (error ’unimplemented-error))
(defun* scan-sexps () "Scan from character number FROM by COUNT balanced expressions.
...
(fn FROM COUNT)"
  (error ’unimplemented-error))
(defun* set-syntax-table () "Select a new syntax table for the current buffer.
One argument, a syntax table.

(fn TABLE)"
  (error ’unimplemented-error))
(defun* skip-chars-backward () "Move point backward, stopping after a char not in STRING, or at pos LIM.
See ‘skip-chars-forward’ for details.
Returns the distance traveled, either zero or negative.

(fn STRING &optional LIM)"
  (error ’unimplemented-error))
(defun* skip-chars-forward () "Move point forward, stopping before a char not in STRING, or at pos LIM.
...
(fn STRING &optional LIM)"
  (error ’unimplemented-error))
(defun* skip-syntax-backward () "Move point backward across chars in specified syntax classes.
...
(fn SYNTAX &optional LIM)"
  (error ’unimplemented-error))
(defun* skip-syntax-forward () "Move point forward across chars in specified syntax classes.
SYNTAX is a string of syntax code characters.
...
(fn SYNTAX &optional LIM)"
  (error ’unimplemented-error))
(defun* standard-syntax-table () "Return the standard syntax table.
This is the one used for new buffers.

(fn)"
  (error ’unimplemented-error))
(defun* string-to-syntax () "Convert a syntax descriptor STRING into a raw syntax descriptor.
...
(fn STRING)"
  (error ’unimplemented-error))
(defun* syntax-class-to-char () "Return the syntax char of CLASS, described by an integer.
For example, if SYNTAX is word constituent (the integer 2), the
character ‘w’ (119) is returned.

(fn SYNTAX)"
  (error ’unimplemented-error))
(defun* syntax-table () "Return the current syntax table.
This is the one specified by the current buffer.

(fn)"
  (error ’unimplemented-error))
(defun* syntax-table-p () "Return t if OBJECT is a syntax table.
