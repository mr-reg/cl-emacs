    ...
    (cond
      ...
      ((eq type #\A)
       (let* ((len (lisp-binary:read-integer 8 stream))
         ...
         (babel:octets-to-string bytes :encoding *string-encoding*)))
      ((eq type #\S)
       (let* ((len (lisp-binary:read-integer 8 stream))
              (bytes (lisp-binary:read-bytes len stream))
              (str (babel:octets-to-string bytes :encoding *string-encoding*)))
      ...
      ((eq type #\C)
       (let ((addr (lisp-binary:read-integer 8 stream))
         ...
         (if (and (symbolp (car cons))
                  (string= (symbol-name (car cons)) "ALIEN-VALUE"))
             (progn
               (log-debug "retrieve alien value #~a" (cdr cons))
               (aref *alien-values* (cdr cons)))
             cons)
         ))
      ((eq type #\R)
       (let ((addr (lisp-binary:read-integer 8 stream)))
...
(defvar *alien-values* (make-array 0 :fill-pointer t :adjustable t))

(defun push-alien-value (obj)
  (let (idx)
    (bt:with-lock-held (*alien-values-lock*)
      (setq idx (vector-push-extend obj *alien-values*)))
    idx))
...
(defun write-lisp-binary-object (obj stream stack)
  ...
  (cond
    ...
    ((stringp obj)
     ...
     (let ((bytes (babel:string-to-octets obj :encoding *string-encoding*)))
    ...
    ((symbolp obj)
     ...
     (let ((bytes (babel:string-to-octets (elisp-symbol-to-string obj) :encoding *string-encoding*)))
       (lisp-binary:write-integer (length bytes) 8 stream)
       (lisp-binary:write-bytes bytes stream)))
    ((hash-table-p obj)
