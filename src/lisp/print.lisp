...
(defun* flush-standard-output ()
  #M"Flush standard-output.
This can be useful after using ‘princ' and the like in scripts.

(fn)"
  (error 'unimplemented-error))
(defun* prin1 ()
  #M"Output the printed representation of OBJECT, any Lisp object.
...
(defun* prin1-to-string ()
  #M"Return a string containing the printed representation of OBJECT.
...
(fn OBJECT &optional PRINTCHARFUN)"
  (error 'unimplemented-error))
(defun* print ()
  #M"Output the printed representation of OBJECT, with newlines around it.
Quoting characters are printed when needed to make output that ‘read'
can handle, whenever this is possible.  For complex objects, the behavior
is controlled by ‘print-level' and ‘print-length', which see.
...
(fn OBJECT &optional PRINTCHARFUN)"
  (error 'unimplemented-error))
(defun* print--preprocess ()
  #M"Extract sharing info from OBJECT needed to print it.
Fills ‘print-number-table' if ‘print-circle' is non-nil.  Does nothing
if ‘print-circle' is nil.

(fn OBJECT)"
  (error 'unimplemented-error))
(defun* redirect-debugging-output ()
  #M"Redirect debugging output (stderr stream) to file FILE.
...
(fn FILE &optional APPEND)"
  (error 'unimplemented-error))
(defun* terpri ()
  #M"Output a newline to stream PRINTCHARFUN.
...
(fn &optional PRINTCHARFUN ENSURE)"
  (error 'unimplemented-error))
(defun* write-char ()
  #M"Output character CHARACTER to stream PRINTCHARFUN.
