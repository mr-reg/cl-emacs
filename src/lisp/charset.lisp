...
(defun* char-charset () "Return the charset of highest priority that contains CH.
...
(defun* charset-after () "Return charset of a character in the current buffer at position POS.
If POS is nil, it defaults to the current point.
If POS is out of range, the value is nil.

(fn &optional POS)"
  (error ’unimplemented-error))
(defun* charset-id-internal () "Internal use only.
Return charset identification number of CHARSET.

(fn &optional CHARSET)"
  (error ’unimplemented-error))
(defun* charset-plist () "Return the property list of CHARSET.

(fn CHARSET)"
  (error ’unimplemented-error))
(defun* charset-priority-list () "Return the list of charsets ordered by priority.
HIGHESTP non-nil means just return the highest priority one.

(fn &optional HIGHESTP)"
  (error ’unimplemented-error))
(defun* charsetp () "Return non-nil if and only if OBJECT is a charset.

(fn OBJECT)"
  (error ’unimplemented-error))
(defun* clear-charset-maps () "Internal use only.
Clear temporary charset mapping tables.
It should be called only from temacs invoked for dumping.

(fn)"
  (error ’unimplemented-error))
(defun* declare-equiv-charset () "Declare an equivalent charset for ISO-2022 decoding.
...
(fn DIMENSION CHARS FINAL-CHAR CHARSET)"
  (error ’unimplemented-error))
(defun* decode-char () "Decode the pair of CHARSET and CODE-POINT into a character.
...
(fn CHARSET CODE-POINT)"
  (error ’unimplemented-error))
(defun* define-charset-alias () "Define ALIAS as an alias for charset CHARSET.

(fn ALIAS CHARSET)"
  (error ’unimplemented-error))
(defun* define-charset-internal () "For internal use only.

(fn ...)"
  (error ’unimplemented-error))
(defun* encode-char () "Encode the character CH into a code-point of CHARSET.
Return the encoded code-point as an integer,
or nil if CHARSET doesn’t support CH.

(fn CH CHARSET)"
  (error ’unimplemented-error))
(defun* find-charset-region () "Return a list of charsets in the region between BEG and END.
...
(fn BEG END &optional TABLE)"
  (error ’unimplemented-error))
(defun* find-charset-string () "Return a list of charsets in STR.
Optional arg TABLE if non-nil is a translation table to look up.
...
(fn STR &optional TABLE)"
  (error ’unimplemented-error))
(defun* get-unused-iso-final-char () "Return an unused ISO final char for a charset of DIMENSION and CHARS.
...
(fn DIMENSION CHARS)"
  (error ’unimplemented-error))
(defun* iso-charset () "Return charset of ISO’s specification DIMENSION, CHARS, and FINAL-CHAR.
...
(fn DIMENSION CHARS FINAL-CHAR)"
  (error ’unimplemented-error))
(defun* make-char () "Return a character of CHARSET whose position codes are CODEn.
...
(fn CHARSET &optional CODE1 CODE2 CODE3 CODE4)"
  (error ’unimplemented-error))
(defun* map-charset-chars () "Call FUNCTION for all characters in CHARSET.
...
(fn FUNCTION CHARSET &optional ARG FROM-CODE TO-CODE)"
  (error ’unimplemented-error))
(defun* set-charset-plist () "Set CHARSET’s property list to PLIST.

(fn CHARSET PLIST)"
  (error ’unimplemented-error))
(defun* set-charset-priority () "Assign higher priority to the charsets given as arguments.

(fn &rest charsets)"
  (error ’unimplemented-error))
(defun* sort-charsets () "Sort charset list CHARSETS by a priority of each charset.
Return the sorted list.  CHARSETS is modified by side effects.
See also ‘charset-priority-list’ and ‘set-charset-priority’.

(fn CHARSETS)"
  (error ’unimplemented-error))
(defun* split-char () "Return list of charset and one to four position-codes of CH.
...
(fn CH)"
  (error ’unimplemented-error))
(defun* unify-charset () "Unify characters of CHARSET with Unicode.
