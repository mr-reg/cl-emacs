...
(defun* fontset-font () "Return a font name pattern for character CH in fontset NAME.
...
(defun* fontset-info () "Return information about a fontset FONTSET on frame FRAME.
...
(fn FONTSET &optional FRAME)"
  (error ’unimplemented-error))
(defun* fontset-list () "Return a list of all defined fontset names.

(fn)"
  (error ’unimplemented-error))
(defun* new-fontset () "Create a new fontset NAME from font information in FONTLIST.
...
(fn NAME FONTLIST)"
  (error ’unimplemented-error))
(defun* query-fontset () "Return the name of a fontset that matches PATTERN.
...
(fn PATTERN &optional REGEXPP)"
  (error ’unimplemented-error))
(defun* set-fontset-font () "Modify FONTSET to use font specification in FONT-SPEC for displaying CHARACTERS.
