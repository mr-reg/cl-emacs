...
(defun* file-locked-p () "Return a value indicating whether FILENAME is locked.
The value is nil if the FILENAME is not locked,
t if it is locked by you, else a string saying which user has locked it.

(fn FILENAME)"
  (error ’unimplemented-error))
(defun* lock-buffer () "Lock FILE, if current buffer is modified.
...
(fn &optional FILE)"
  (error ’unimplemented-error))
(defun* lock-file () "Lock FILE.
If the option ‘create-lockfiles’ is nil, this does nothing.

(fn FILE)"
  (error ’unimplemented-error))
(defun* unlock-buffer () "Unlock the file visited in the current buffer.
