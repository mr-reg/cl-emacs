...
(defun* abort-minibuffers () "Abort the current minibuffer.
...
(defun* active-minibuffer-window () "Return the currently active minibuffer window, or nil if none.

(fn)"
  (error ’unimplemented-error))
(defun* all-completions () "Search for partial matches of STRING in COLLECTION.
...
(fn STRING COLLECTION &optional PREDICATE HIDE-SPACES)"
  (error ’unimplemented-error))
(defun* assoc-string () "Like ‘assoc’ but specifically for strings (and symbols).
...
(fn KEY LIST &optional CASE-FOLD)"
  (error ’unimplemented-error))
(defun* completing-read () "Read a string in the minibuffer, with completion.
...
(fn PROMPT COLLECTION &optional PREDICATE REQUIRE-MATCH INITIAL-INPUT HIST DEF INHERIT-INPUT-METHOD)"
  (error ’unimplemented-error))
(defun* innermost-minibuffer-p () "Return t if BUFFER is the most nested active minibuffer.
No argument or nil as argument means use the current buffer as BUFFER.

(fn &optional BUFFER)"
  (error ’unimplemented-error))
(defun* internal-complete-buffer () "Perform completion on buffer names.
...
(fn STRING PREDICATE FLAG)"
  (error ’unimplemented-error))
(defun* minibuffer-contents () "Return the user input in a minibuffer as a string.
If the current buffer is not a minibuffer, return its entire contents.

(fn)"
  (error ’unimplemented-error))
(defun* minibuffer-contents-no-properties () "Return the user input in a minibuffer as a string, without text-properties.
If the current buffer is not a minibuffer, return its entire contents.

(fn)"
  (error ’unimplemented-error))
(defun* minibuffer-depth () "Return current depth of activations of minibuffer, a nonnegative integer.

(fn)"
  (error ’unimplemented-error))
(defun* minibuffer-innermost-command-loop-p () "Return t if BUFFER is a minibuffer at the current command loop level.
No argument or nil as argument means use the current buffer as BUFFER.

(fn &optional BUFFER)"
  (error ’unimplemented-error))
(defun* minibuffer-prompt () "Return the prompt string of the currently-active minibuffer.
If no minibuffer is active, return nil.

(fn)"
  (error ’unimplemented-error))
(defun* minibuffer-prompt-end () "Return the buffer position of the end of the minibuffer prompt.
Return (point-min) if current buffer is not a minibuffer.

(fn)"
  (error ’unimplemented-error))
(defun* minibufferp () "Return t if BUFFER is a minibuffer.
...
(fn &optional BUFFER LIVE)"
  (error ’unimplemented-error))
(defun* read-buffer () "Read the name of a buffer and return it as a string.
...
(fn PROMPT &optional DEF REQUIRE-MATCH PREDICATE)"
  (error ’unimplemented-error))
(defun* read-command () "Read the name of a command and return as a symbol.
...
(fn PROMPT &optional DEFAULT-VALUE)"
  (error ’unimplemented-error))
(defun* read-from-minibuffer () "Read a string from the minibuffer, prompting with string PROMPT.
...
(fn PROMPT &optional INITIAL-CONTENTS KEYMAP READ HIST DEFAULT-VALUE INHERIT-INPUT-METHOD)"
  (error ’unimplemented-error))
(defun* read-string () "Read a string from the minibuffer, prompting with string PROMPT.
...
(fn PROMPT &optional INITIAL-INPUT HISTORY DEFAULT-VALUE INHERIT-INPUT-METHOD)"
  (error ’unimplemented-error))
(defun* read-variable () "Read the name of a user option and return it as a symbol.
Prompt with PROMPT.  By default, return DEFAULT-VALUE or its first element
...
(fn PROMPT &optional DEFAULT-VALUE)"
  (error ’unimplemented-error))
(defun* set-minibuffer-window () "Specify which minibuffer window to use for the minibuffer.
This affects where the minibuffer is displayed if you put text in it
without invoking the usual minibuffer commands.

(fn WINDOW)"
  (error ’unimplemented-error))
(defun* test-completion () "Return non-nil if STRING is a valid completion.
...
(fn STRING COLLECTION &optional PREDICATE)"
  (error ’unimplemented-error))
(defun* try-completion () "Return longest common substring of all completions of STRING in COLLECTION.
