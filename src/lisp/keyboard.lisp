...
(defun* clear-this-command-keys ()
  #M"Clear out the vector that ‘this-command-keys' returns.
...
(defun* command-error-default-function ()
  #M"Produce default output for unhandled error message.
Default value of ‘command-error-function'.

(fn DATA CONTEXT SIGNAL)"
  (error 'unimplemented-error))
(defun* current-idle-time ()
  #M"Return the current length of Emacs idleness, or nil.
...
(defun* current-input-mode ()
  #M"Return information about the way Emacs currently reads keyboard input.
...
(fn)"
  (error 'unimplemented-error))
(defun* discard-input ()
  #M"Discard the contents of the terminal input buffer.
Also end any kbd macro being defined.

(fn)"
  (error 'unimplemented-error))
(defun* event-convert-list ()
  #M"Convert the event description list EVENT-DESC to an event type.
...
(defun* exit-recursive-edit ()
  #M"Exit from the innermost recursive edit or minibuffer.

(fn)"
  (error 'unimplemented-error))
(defun* input-pending-p ()
  #M"Return t if command input is currently available with no wait.
...
(fn &optional CHECK-TIMERS)"
  (error 'unimplemented-error))
(defun* internal--track-mouse ()
  #M"Call BODYFUN with mouse movement events enabled.

(fn BODYFUN)"
  (error 'unimplemented-error))
(defun* internal-event-symbol-parse-modifiers ()
  #M"Parse the event symbol.  For internal use.

(fn SYMBOL)"
  (error 'unimplemented-error))
(defun* internal-handle-focus-in ()
  #M"Internally handle focus-in events.
This function potentially generates an artificial switch-frame event.

(fn EVENT)"
  (error 'unimplemented-error))
(defun* lossage-size ()
  #M"Return or set the maximum number of keystrokes to save.
...
(fn &optional ARG)"
  (error 'unimplemented-error))
(defun* open-dribble-file ()
  #M"Start writing input events to a dribble file called FILE.
...
(fn FILE)"
  (error 'unimplemented-error))
(defun* posn-at-point ()
  #M"Return position information for buffer position POS in WINDOW.
...
(fn &optional POS WINDOW)"
  (error 'unimplemented-error))
(defun* posn-at-x-y ()
  #M"Return position information for pixel coordinates X and Y.
...
(defun* read-key-sequence ()
  #M"Read a sequence of keystrokes and return as a string or vector.
...
(defun* read-key-sequence-vector ()
  #M"Like ‘read-key-sequence' but always return a vector.

(fn PROMPT &optional CONTINUE-ECHO DONT-DOWNCASE-LAST CAN-RETURN-SWITCH-FRAME CMD-LOOP)"
  (error 'unimplemented-error))
(defun* recent-keys ()
  #M"Return vector of last few events, not counting those from keyboard macros.
...
(defun* recursion-depth ()
  #M"Return the current depth in recursive edits.

(fn)"
  (error 'unimplemented-error))
(defun* recursive-edit ()
  #M"Invoke the editor command loop recursively.
...
(fn)"
  (error 'unimplemented-error))
(defun* set--this-command-keys ()
  #M"Set the vector to be returned by ‘this-command-keys'.
...
(defun* set-input-interrupt-mode ()
  #M"Set interrupt mode of reading keyboard input.
...
(defun* set-input-meta-mode ()
  #M"Enable or disable 8-bit input on TERMINAL.
...
(defun* set-input-mode ()
  #M"Set mode of reading keyboard input.
...
(defun* set-output-flow-control ()
  #M"Enable or disable ^S/^Q flow control for output to TERMINAL.
...
(fn FLOW &optional TERMINAL)"
  (error 'unimplemented-error))
(defun* set-quit-char ()
  #M"Specify character used for quitting.
...
(fn QUIT)"
  (error 'unimplemented-error))
(defun* suspend-emacs ()
  #M"Stop Emacs and return to superior process.  You can resume later.
...
(defun* this-command-keys ()
  #M"Return the key sequence that invoked this command.
...
(defun* this-command-keys-vector ()
  #M"Return the key sequence that invoked this command, as a vector.
However, if the command has called ‘read-key-sequence', it returns
the last key sequence that has been read.
...
(defun* this-single-command-raw-keys ()
  #M"Return the raw events that were read for this command.
More generally, it returns the last key sequence read, either by
the command loop or by ‘read-key-sequence'.
...
(fn)"
  (error 'unimplemented-error))
(defun* top-level ()
  #M"Exit all recursive editing levels.
