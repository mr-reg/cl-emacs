...
(defun* bidi-find-overridden-directionality () "Return position between FROM and TO where directionality was overridden.
...
(defun* bidi-resolved-levels () "Return the resolved bidirectional levels of characters at VPOS.
...
(fn &optional VPOS)"
  (error ’unimplemented-error))
(defun* buffer-text-pixel-size () "Return size of whole text of BUFFER-OR-NAME in WINDOW.
...
(fn &optional BUFFER-OR-NAME WINDOW X-LIMIT Y-LIMIT)"
  (error ’unimplemented-error))
(defun* current-bidi-paragraph-direction () "Return paragraph direction at point in BUFFER.
...
(fn &optional BUFFER)"
  (error ’unimplemented-error))
(defun* display--line-is-continued-p () "Return non-nil if the current screen line is continued on display.

(fn)"
  (error ’unimplemented-error))
(defun* format-mode-line () "Format a string out of a mode line format specification.
...
(fn FORMAT &optional FACE WINDOW BUFFER)"
  (error ’unimplemented-error))
(defun* get-display-property () "Get the value of the ‘display’ property PROP at POSITION.
...
(fn POSITION PROP &optional OBJECT PROPERTIES)"
  (error ’unimplemented-error))
(defun* invisible-p () "Non-nil if text properties at POS cause text there to be currently invisible.
...
(fn POS)"
  (error ’unimplemented-error))
(defun* line-pixel-height () "Return height in pixels of text line in the selected window.

Value is the height in pixels of the line at point.

(fn)"
  (error ’unimplemented-error))
(defun* long-line-optimizations-p () "Return non-nil if long-line optimizations are in effect in current buffer.
See ‘long-line-threshold’ and ‘large-hscroll-threshold’ for what these
optimizations mean and when they are in effect.

(fn)"
  (error ’unimplemented-error))
(defun* lookup-image-map () "Lookup in image map MAP coordinates X and Y.
...
(fn MAP X Y)"
  (error ’unimplemented-error))
(defun* move-point-visually () "Move point in the visual order in the specified DIRECTION.
...
(fn DIRECTION)"
  (error ’unimplemented-error))
(defun* set-buffer-redisplay () "Mark the current buffer for redisplay.
This function may be passed to ‘add-variable-watcher’.

(fn SYMBOL NEWVAL OP WHERE)"
  (error ’unimplemented-error))
(defun* tab-bar-height () "Return the number of lines occupied by the tab bar of FRAME.
...
(fn &optional FRAME PIXELWISE)"
  (error ’unimplemented-error))
(defun* tool-bar-height () "Return the number of lines occupied by the tool bar of FRAME.
If FRAME is nil or omitted, use the selected frame.  Optional argument
PIXELWISE non-nil means return the height of the tool bar in pixels.

(fn &optional FRAME PIXELWISE)"
  (error ’unimplemented-error))
(defun* window-text-pixel-size () "Return the size of the text of WINDOW’s buffer in pixels.
