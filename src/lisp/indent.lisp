...
(defun* compute-motion () "Scan through the current buffer, calculating screen position.
...
(defun* current-column () "Return the horizontal position of point.  Beginning of line is column 0.
...
(fn)"
  (error ’unimplemented-error))
(defun* current-indentation () "Return the indentation of the current line.
...
(fn)"
  (error ’unimplemented-error))
(defun* indent-to () "Indent from point with tabs and spaces until COLUMN is reached.
...
(fn COLUMN &optional MINIMUM)"
  (error ’unimplemented-error))
(defun* line-number-display-width () "Return the width used for displaying line numbers in the selected window.
...
(fn &optional PIXELWISE)"
  (error ’unimplemented-error))
(defun* move-to-column () "Move point to column COLUMN in the current line.
...
(fn COLUMN &optional FORCE)"
  (error ’unimplemented-error))
(defun* vertical-motion () "Move point to start of the screen line LINES lines down.
