...
(defun* controlling-tty-p () "Return non-nil if TERMINAL is the controlling tty of the Emacs process.
...
(defun* resume-tty () "Resume the previously suspended terminal device TTY.
...
(fn &optional TTY)"
  (error ’unimplemented-error))
(defun* suspend-tty () "Suspend the terminal device TTY.
...
(fn &optional TTY)"
  (error ’unimplemented-error))
(defun* tty--output-buffer-size () "Return the output buffer size of TTY.
...
(fn &optional TTY)"
  (error ’unimplemented-error))
(defun* tty--set-output-buffer-size () "Set the output buffer size for a TTY.
...
(fn SIZE &optional TTY)"
  (error ’unimplemented-error))
(defun* tty-display-color-cells () "Return the number of colors supported by the tty device TERMINAL.
...
(fn &optional TERMINAL)"
  (error ’unimplemented-error))
(defun* tty-display-color-p () "Return non-nil if the tty device TERMINAL can display colors.
...
(fn &optional TERMINAL)"
  (error ’unimplemented-error))
(defun* tty-no-underline () "Declare that the tty used by TERMINAL does not handle underlining.
...
(fn &optional TERMINAL)"
  (error ’unimplemented-error))
(defun* tty-top-frame () "Return the topmost terminal frame on TERMINAL.
TERMINAL can be a terminal object, a frame or nil (meaning the
...
(fn &optional TERMINAL)"
  (error ’unimplemented-error))
(defun* tty-type () "Return the type of the tty device that TERMINAL uses.
