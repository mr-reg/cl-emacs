...
(defun* clear-composition-cache () "Internal use only.
Clear composition cache.

(fn)"
  (error ’unimplemented-error))
(defun* compose-region-internal () "Internal use only.
...
(fn START END &optional COMPONENTS MODIFICATION-FUNC)"
  (error ’unimplemented-error))
(defun* compose-string-internal () "Internal use only.
...
(fn STRING START END &optional COMPONENTS MODIFICATION-FUNC)"
  (error ’unimplemented-error))
(defun* composition-get-gstring () "Return a glyph-string for characters between FROM and TO.
...
(fn FROM TO FONT-OBJECT STRING)"
  (error ’unimplemented-error))
(defun* composition-sort-rules () "Sort composition RULES by their LOOKBACK parameter.
...
(fn RULES)"
  (error ’unimplemented-error))
(defun* find-composition-internal () "Internal use only.
