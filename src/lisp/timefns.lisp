...
(defun* current-cpu-time ()
  #M"Return the current CPU time along with its resolution.
...
(defun* current-time ()
  #M"Return the current time, as the number of seconds since 1970-01-01 00:00:00.
...
(defun* current-time-string ()
  #M"Return the current local time, as a human-readable string.
...
(defun* current-time-zone ()
  #M"Return the offset and name for the local time zone.
...
(fn &optional SPECIFIED-TIME ZONE)"
  (error 'unimplemented-error))
(defun* decode-time ()
  #M"Decode a timestamp into (SEC MINUTE HOUR DAY MONTH YEAR DOW DST UTCOFF).
...
(fn TIME &rest OBSOLESCENT-ARGUMENTS)"
  (error 'unimplemented-error))
(defun* float-time ()
  #M"Return the current time, as a float number of seconds since the epoch.
...
(defun* set-time-zone-rule ()
  #M"Set the Emacs local time zone using TZ, a string specifying a time zone rule.
...
(fn TZ)"
  (error 'unimplemented-error))
(defun* time-add ()
  #M"Return the sum of two time values A and B, as a time value.
...
(fn A B)"
  (error 'unimplemented-error))
(defun* time-convert ()
  #M"Convert TIME value to a Lisp timestamp of the given FORM.
...
(fn TIME &optional FORM)"
  (error 'unimplemented-error))
(defun* time-equal-p ()
  #M"Return non-nil if A and B are equal time values.
See ‘format-time-string' for the various forms of a time value.

(fn A B)"
  (error 'unimplemented-error))
(defun* time-less-p ()
  #M"Return non-nil if time value A is less than time value B.
See ‘format-time-string' for the various forms of a time value.
For example, nil stands for the current time.
...
(fn A B)"
  (error 'unimplemented-error))
(defun* time-subtract ()
  #M"Return the difference between two time values A and B, as a time value.
